        ...
        if(!(sender instanceof Player)){
            ...
            if (args.length == 0) {
                ...
                if(isEnabled){
                    ...
                    plugin.saveConfig();
                    plugin.reloadConfig();
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&3&lITEMRARITYGLOW &eYou have toggled the glow &cOFF&e."));

                }else{
                    plugin.getConfig().set("Enabled",true);
                    plugin.saveConfig();
                    plugin.reloadConfig();
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&3&lITEMRARITYGLOW &eYou have toggled the glow &aON&e."));
                }
            }
        }
        return false;
